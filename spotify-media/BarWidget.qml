...
Item {
  ...
  readonly property string visualizerType: (ps.barVisualizer !== undefined && String(ps.barVisualizer) !== "") ? String(ps.barVisualizer) : "linear" // linear | mirrored | wave | none
  ...
  readonly property real minWidth: (ps.minWidth !== undefined) ? Number(ps.minWidth) : Math.max(170, iconSize + 60)
  ...
  readonly property real contentWidth: {
    ...
    if (isVertical) return verticalSize;

    
    var artVisible = hasPlayer && ((showAlbumArt === true) || (showProgressRing === true));

    
    var s = Style.marginS * effectiveScaling;

    
    var fixed = iconSize + iconSize  + (artVisible ? artSize : 0);

    
    
    var gaps = artVisible ? 3 : 2;

    
    var textW = 0;
    if (titleMetrics.contentWidth > 0)
        textW = titleMetrics.contentWidth + (Style.marginXXS * 2);

    
    var innerMargins = Style.marginS * effectiveScaling;

    var total = fixed + (gaps * s) + textW + innerMargins;

    if (useFixedWidth)
        return minWidth;

    
    return Math.min(total, minWidth);
    }


  // Cava
  readonly property string cavaComponentId: "bar:spotify-media:" + (screen && screen.name ? screen.name : "noscreen") + ":" + section + ":" + sectionWidgetIndex

  readonly property bool needsCava: (showVisualizer === true) && (visualizerType !== "") && (visualizerType !== "none")
  ...
  Rectangle {
    ...
    width: isVertical ? (isHidden ? 0 : verticalSize) : contentWidth
    ...
    Item {
      ...
      MouseArea {
        ...
        onClicked: function(mouse) {
          if (mouse.button === Qt.LeftButton && hasPlayer && player.canTogglePlaying) {
